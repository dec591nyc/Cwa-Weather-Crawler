...
import type { ApiSource, ApiSourcesResponse, CountySummary, CountySummaryResponse, DataLayer, EarthquakeDisplayMode, EarthquakeEvent, EarthquakeResponse, GeoJsonCollection, GeoJsonFeature, HealthResponse, ObservationMetric, Pm25Observation, SyncStatusResponse } from "./types/weather.ts";
...
export const App: React.FC = () => {
  ...
  const [earthquakeTotalCount, setEarthquakeTotalCount] = useState<number>(0);
  const [selectedEarthquakeKey, setSelectedEarthquakeKey] = useState<string | null>(null);
  const [earthquakeDisplayMode, setEarthquakeDisplayMode] = useState<EarthquakeDisplayMode>("selected");
  ...
  const selectedEarthquake = useMemo(() => filteredEarthquakes.find((event) => event.earthquake_key === selectedEarthquakeKey) || filteredEarthquakes[0] || null, [filteredEarthquakes, selectedEarthquakeKey]);
  const earthquakeMapEvents = useMemo(() => earthquakeDisplayMode === "selected" ? (selectedEarthquake ? [selectedEarthquake] : []) : filteredEarthquakes, [earthquakeDisplayMode, filteredEarthquakes, selectedEarthquake]);
  ...
  const fetchData = async () => {
    try {
      ...
      if (earthquakesRes.ok) { const earthquakeData: EarthquakeResponse = await earthquakesRes.json(); const earthquakeRows = earthquakeData.earthquakes || []; setEarthquakes(earthquakeRows); setEarthquakeTotalCount(earthquakeData.count || earthquakeRows.length || 0); setSelectedEarthquakeKey((current) => current || earthquakeRows[0]?.earthquake_key || null); }
  ...
  const loadInitialData = async () => { setLoading(true); await fetchData(); };
  useEffect(() => { void loadInitialData(); }, []);
  useEffect(() => { if (activePage === "windy") setWindyMounted(true); }, [activePage]);
  useEffect(() => { if (filteredEarthquakes.length && !filteredEarthquakes.some((event) => event.earthquake_key === selectedEarthquakeKey)) setSelectedEarthquakeKey(filteredEarthquakes[0].earthquake_key); }, [filteredEarthquakes, selectedEarthquakeKey]);
  ...
  const handleMetricChange = (metric: ObservationMetric) => { setSelectedCounty(""); setActiveMetric(metric); const config = metricConfigs[metric]; setMetricMinByMetric((current) => ({ ...current, [metric]: current[metric] ?? config.min })); };
  const handleEarthquakeSelect = (earthquakeKey: string) => { setSelectedEarthquakeKey(earthquakeKey); setActiveDataLayer("earthquakes"); setEarthquakeDisplayMode("selected"); };
