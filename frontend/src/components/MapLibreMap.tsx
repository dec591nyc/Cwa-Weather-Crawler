...
function popupHtml(props: Record<string, any>): string {
  ...
  return `<div class="popup-container"><div class="popup-header"><div class="popup-station-name">${props.station_name || "-"}</div><div class="popup-location-sub">${props.county || ""} ${props.town || ""} · ${sourceLabel}</div></div><div class="popup-metric-large">${props.metricLabel || "觀測"} ${props.displayValue || "-"}</div><div class="popup-grid"><span class="popup-label">氣溫</span><span class="popup-value">${props.temperature ?? "-"} °C</span><span class="popup-label">短週期降雨量</span><span class="popup-value">${props.rainfall_10min ?? "-"} mm</span><span class="popup-label">當天累積雨量</span><span class="popup-value">${props.rainfall_today ?? "-"} mm</span><span class="popup-label">濕度</span><span class="popup-value">${props.humidity ?? "-"}%</span><span class="popup-label">風速</span><span class="popup-value">${props.wind_speed ?? "-"} m/s</span><span class="popup-label">能見度</span><span class="popup-value">${visibilityText}</span></div><div class="popup-time">觀測時間: ${observedText}</div></div>`;
}
