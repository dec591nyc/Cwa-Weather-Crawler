...
export const CountySummaryPanel: React.FC<CountySummaryPanelProps> = ({
  summaries,
  counties,
  features,
  pm25Observations,
  ...
  const metricPoints = useMemo(
    () => buildMetricPoints(features, pm25Observations, activeMetric, selectedCounty),
    [features, pm25Observations, activeMetric, selectedCounty]
  );
  const metricStats = useMemo(() => summarizePoints(metricPoints), [metricPoints]);
  const highestRanking = useMemo(
    () => getRanking(metricPoints, "highest"),
    [metricPoints]
  );
  const lowestRanking = useMemo(
    () => getRanking(metricPoints, "lowest"),
    [metricPoints]
  );
    ...
    <section className="insight-dock" aria-label="測站觀測統計">
      <div className="insight-overview" aria-live="polite">
        <div className="insight-overview-header">
          <div>
            <p className="insight-kicker">{scopeLabel}</p>
            <h2 className="insight-title">{metricConfig.label}測站統計</h2>
          </div>
        ...
        <div className="summary-metrics">
          <div>
            <span>平均</span>
            <strong>{formatMetricValue(activeMetric, metricStats.avg)}</strong>
            <small className="summary-metric-note">{stationScopeText}平均</small>
          </div>
          <div>
            <span>最高</span>
            <strong>{formatMetricValue(activeMetric, metricStats.max?.value ?? null)}</strong>
            <small className="summary-metric-note">{metricStats.max?.stationName ?? "無站名"}</small>
          </div>
          <div>
            <span>最低</span>
            <strong>{formatMetricValue(activeMetric, metricStats.min?.value ?? null)}</strong>
            <small className="summary-metric-note">{metricStats.min?.stationName ?? "無站名"}</small>
          </div>
          <div>
            <span>有效站點</span>
            <strong>{metricStats.count}</strong>
            <small className="summary-metric-note">{metricConfig.source === "weather" ? "CWA station" : "MOENV station"}</small>
          </div>
        </div>
        <div className="summary-footnote">
          ...
          <span>摘要依最新批次測站值計算</span>
        </div>
      </div>

      <div className="ranking-block metric-ranking-block">
        <div className="insight-block-heading">
          <h3>{metricConfig.label}排行榜 Top 5</h3>
        </div>
        {metricPoints.length ? (
          <div className="ranking-columns">
            <div className="ranking-column">
              <div className="ranking-column-title">
                <strong>最高{metricConfig.label} Top 5</strong>
                <span>由高到低</span>
              </div>
            ...
            <div className="ranking-column">
              <div className="ranking-column-title">
                <strong>最低{metricConfig.label} Top 5</strong>
                <span>由低到高</span>
              </div>
