...
interface WindyMapPageProps { features: GeoJsonFeature[]; pm25Observations: Pm25Observation[]; selectedCounty: string; activeMetric: ObservationMetric; metricMin: number; isActive: boolean; }
...
export const WindyMapPage: React.FC<WindyMapPageProps> = ({ features, pm25Observations, selectedCounty, activeMetric, metricMin, isActive }) => {
  ...
  const renderMarkers = useCallback((api: WindyApi) => {
    ...
    if (config.source === "airQuality") {
      ...
      setWeatherCount(0); setAirQualityCount(rows.length);
    ...
    setWeatherCount(rows.length); setAirQualityCount(0);
    for (const { feature, value } of rows) {
      ...
      marker.bindPopup(`<div class="windy-popup"><strong>${props.station_name || "-"}</strong><span>${props.county || ""} ${props.town || ""} · 中央氣象署即時觀測</span><div class="popup-metric-large">${config.label} ${formatMetricValue(activeMetric, value)}</div><dl><dt>氣溫</dt><dd>${props.temperature ?? "-"} °C</dd><dt>10 分鐘降雨量</dt><dd>${displayRainfall(props.rainfall_10min)}</dd><dt>過去 24 小時雨量</dt><dd>${displayRainfall(props.rainfall_24h)}</dd><dt>濕度</dt><dd>${props.humidity ?? "-"}%</dd><dt>風速</dt><dd>${props.wind_speed ?? "-"} m/s</dd><dt>能見度</dt><dd>${props.visibility_description || props.visibility_km || "-"}</dd><dt>時間</dt><dd>${observedText(props.observed_at || props.fetched_at)}</dd></dl></div>`);
      marker.addTo(layer);
    }
