...
export const App: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setError(null);
      const [countiesRes, geojsonRes, pm25Res, earthquakesRes, healthRes, sourcesRes, syncStatusRes] = await Promise.all([fetch(apiUrl("/api/summary/counties")), fetch(apiUrl("/api/weather/stations.geojson")), fetch(apiUrl("/api/pm25/latest")), fetch(apiUrl("/api/earthquakes/latest?limit=20")), fetch(apiUrl("/api/health")), fetch(apiUrl("/api/data-sources")), fetch(apiUrl("/api/sync/status"))]);
