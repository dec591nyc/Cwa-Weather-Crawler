...
const osmRasterStyle: maplibregl.StyleSpecification = { version: 8, sources: { "osm-raster": { type: "raster", tiles: ["https://tile.openstreetmap.org/{z}/{x}/{y}.png"], tileSize: 256, maxzoom: 19, attribution: "© OpenStreetMap contributors" } }, layers: [{ id: "osm-raster", type: "raster", source: "osm-raster" }] };
...
function buildObservationFeatures(state: ObservationState): GeoJSON.FeatureCollection {
  ...
  if (config.source === "airQuality") {
    const features = state.pm25Observations.filter((obs) => obs.lat !== null && obs.lon !== null).filter((obs) => !state.selectedCounty || obs.county === state.selectedCounty).map((obs) => ({ obs, value: getAirQualityMetricValue(obs, state.activeMetric) })).filter(({ value }) => value !== null && value >= state.metricMin).map(({ obs, value }) => ({ type: "Feature" as const, geometry: { type: "Point" as const, coordinates: [obs.lon as number, obs.lat as number] }, properties: { sourceType: "airQuality", station_name: obs.station_name || "-", county: obs.county || "", town: "", value, displayValue: formatMetricValue(state.activeMetric, value), color: getMetricColor(state.activeMetric, value), metricLabel: config.label, metricUnit: config.unit, observed_at: obs.observed_at, fetched_at: obs.fetched_at, status: obs.status, pollutant: obs.pollutant, pm25: obs.pm25, pm25_avg: obs.pm25_avg, pm10: obs.pm10, pm10_avg: obs.pm10_avg, o3: obs.o3, o3_8hr: obs.o3_8hr, co: obs.co, co_8hr: obs.co_8hr, no2: obs.no2, so2: obs.so2 } }));
    return { type: "FeatureCollection", features };
  }
  const features = state.features.filter((feature) => !state.selectedCounty || feature.properties.county === state.selectedCounty).map((feature) => ({ feature, value: getWeatherMetricValue(feature.properties, state.activeMetric) })).filter(({ value }) => value !== null && value >= state.metricMin).map(({ feature, value }) => ({ type: "Feature" as const, geometry: feature.geometry, properties: { ...feature.properties, sourceType: "weather", value, displayValue: formatMetricValue(state.activeMetric, value), color: getMetricColor(state.activeMetric, value), metricLabel: config.label, metricUnit: config.unit } }));
  return { type: "FeatureCollection", features };
}
...
export const MapLibreMap: React.FC<MapLibreMapProps> = ({ features, pm25Observations, selectedCounty, activeMetric, metricMin }) => {
  ...
  const updateObservationData = () => {
    ...
    if (map.getLayer("observation-circles")) { map.setPaintProperty("observation-circles", "circle-color", getMapLibreColorExpression(state.activeMetric) as any); map.setPaintProperty("observation-circles", "circle-radius", 7); }
  };
  const ensureObservationLayers = () => {
    ...
    if (!map.getLayer("observation-circles")) map.addLayer({ id: "observation-circles", type: "circle", source: "observation-source", paint: { "circle-radius": 7, "circle-color": getMapLibreColorExpression(stateRef.current.activeMetric) as any, "circle-stroke-width": 1.5, "circle-stroke-color": "#ffffff", "circle-opacity": 0.9 } });
  ...
  useEffect(() => {
    ...
    map.on("click", "observation-circles", (event) => { if (!event.features || event.features.length === 0) return; const geometry = event.features[0].geometry as GeoJSON.Point; const coordinates = geometry.coordinates.slice() as [number, number]; new maplibregl.Popup({ className: "custom-mapbox-popup" }).setLngLat(coordinates).setHTML(popupHtml(event.features[0].properties || {})).addTo(map); });
