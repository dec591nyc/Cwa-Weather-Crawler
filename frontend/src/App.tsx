...
function providerCode(provider: string): string {
  if (provider === "MOENV") return "ENV";
  if (provider === "CWA") return "CWA";
  return provider.slice(0, 3).toUpperCase();
}

function syncLabel(status: SyncStatusResponse | null, fallbackWarning: string | null, error: string | null): string {
  if (error) return "API 連線異常";
  if (!status) return fallbackWarning ? "同步警告" : "系統正常";
  if (status.overall_status === "ok") return "全部同步成功";
  if (status.overall_status === "warning") return "部分同步異常";
  return "核心同步異常";
}

const AppLogo: React.FC = () => (
  <div className="header-logo" aria-label="Taiwan Weather Monitor">
    <svg className="header-logo-mark" viewBox="0 0 36 36" role="img" aria-hidden="true">
      <circle cx="18" cy="18" r="15" fill="currentColor" opacity="0.14" />
      <path d="M10 21.5c2.9-4.4 5.4-6.8 7.6-7.1 2.1-.3 3.7 1 4.9 3.8 1.1-.8 2.4-.9 3.7-.3 1.3.6 2.1 1.7 2.5 3.3" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" />
      <path d="M13 25.5h10.8M11.5 10.5l3 3M24.5 10.2l-2.4 3.2" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
    </svg>
    <span className="header-logo-copy">TWM</span>
  </div>
);
...
export const App: React.FC = () => {
  ...
  const latestObservedAt = useMemo(
    () => latestTimestamp([...features.map((feature) => feature.properties.observed_at), ...pm25Observations.map((obs) => obs.observed_at), ...earthquakes.map((event) => event.earthquake_time)]),
    [features, pm25Observations, earthquakes],
  );
  ...
  const fetchData = async () => {
    try {
      setError(null);
      const [countiesRes, geojsonRes, pm25Res, earthquakesRes, healthRes, sourcesRes, syncStatusRes] = await Promise.all([
        fetch(apiUrl("/api/summary/counties")),
        fetch(apiUrl("/api/weather/stations.geojson")),
        fetch(apiUrl("/api/pm25/latest")),
        fetch(apiUrl("/api/earthquakes/latest?limit=20")),
      ...
      if (syncStatusRes.ok) {
        const statusData: SyncStatusResponse = await syncStatusRes.json();
        setSyncStatus(statusData);
        setSyncWarning(statusData.overall_status === "ok" ? null : statusData.summary);
      }
    } catch (err: any) {
      console.error(err);
  ...
  const syncLatestObservations = async () => {
    try {
      ...
      if (!refreshRes.ok) throw new Error("Observation sync failed");
      const refreshData = await refreshRes.json();
      setSyncWarning(refreshData.status === "partial_failed" ? "部分資料來源同步失敗" : null);
    } catch (err) {
      console.warn(err);
  ...
  useEffect(() => {
    if (activePage === "windy") setWindyMounted(true);
  }, [activePage]);
    ...
    <>
      <header className="header-bar">
        <div className="header-title-container">
          <AppLogo />
          <div>
            <h1 className="header-title">台灣即時氣象、空品與地震觀測</h1>
            <p className="header-subtitle">CWA 觀測、地震震央與環境部污染物指標</p>
            <div className="brand-strip" aria-label="數據 API 與地圖來源">
              ...
              <span className="brand-badge brand-maplibre">MapLibre</span>
            </div>
          </div>
        </div>
                ...
                {sourceGroups.map((group) => {
                  ...
                  return (
                    <button key={group.provider} type="button" className={`api-source-row ${pinned ? "active" : ""}`} onMouseEnter={() => setHoveredSourceProvider(group.provider)} onClick={() => setPinnedSourceProvider(pinned ? null : group.provider)}>
                      <span className={`source-provider-mark source-provider-${group.provider.toLowerCase()}`}>{providerCode(group.provider)}</span>
                      <span className="api-source-name">{formatProviderLabel(group.provider)}</span>
        ...
        <div className="header-actions">
          <a className="github-btn" href={GITHUB_URL} target="_blank" rel="noreferrer" aria-label="Open GitHub repository">
            <span>GitHub</span>
          </a>
          <div className="header-status">
            <span className={`status-dot ${error || (syncStatus && syncStatus.overall_status !== "ok") || syncWarning ? "stale" : ""}`} />
      ...
      <main className="app-container">
        <section className="map-workspace" aria-label="台灣即時觀測地圖">
            ...
            {loading ? (
              ...
              <>
                <div style={mapLayerStyle(activePage === "dashboard")}>
                  {activeDataLayer === "earthquakes" ? (
                    <EarthquakeMap earthquakes={earthquakes} />
                  ) : (
                    <>
                      <MapLibreMap features={features} pm25Observations={pm25Observations} selectedCounty={selectedCounty} activeMetric={activeMetric} metricMin={metricMin} />
                ...
                {windyMounted && (
                  <div style={mapLayerStyle(activePage === "windy")}>
                    <WindyMapPage features={features} pm25Observations={pm25Observations} earthquakes={earthquakes} dataLayer={activeDataLayer} selectedCounty={selectedCounty} activeMetric={activeMetric} metricMin={metricMin} isActive={activePage === "windy"} />
                  </div>
                )}
              </>
            )}
        ...
        <LayerControl dataLayer={activeDataLayer} onDataLayerChange={setActiveDataLayer} activeMetric={activeMetric} onMetricChange={handleMetricChange} metricMin={metricMin} onMetricMinChange={handleMetricMinChange} />
        {activeDataLayer === "earthquakes" ? (
