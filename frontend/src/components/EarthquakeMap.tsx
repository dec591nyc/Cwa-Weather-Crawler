...
function initialMapView(earthquakes: EarthquakeEvent[]): { center: [number, number]; zoom: number } {
  const first = earthquakes[0];
  if (
    earthquakes.length === 1 &&
    first?.epicenter_lon !== null &&
    first?.epicenter_lon !== undefined &&
    first?.epicenter_lat !== null &&
    first?.epicenter_lat !== undefined
  ) {
    return { center: [first.epicenter_lon, first.epicenter_lat], zoom: 8.2 };
  }
  return { center: [121.0, 23.7], zoom: 7.0 };
}
...
export const EarthquakeMap: React.FC<EarthquakeMapProps> = ({ earthquakes }) => {
  ...
  const ensureLayers = () => {
    ...
    if (!map.getSource("earthquake-epicenters")) {
      map.addSource("earthquake-epicenters", { type: "geojson", data: buildEpicenterFeatures(earthquakesRef.current) });
    }
    if (!map.getSource("earthquake-stations")) {
      map.addSource("earthquake-stations", { type: "geojson", data: buildStationFeatures(earthquakesRef.current[0]?.stations || []) });
    }
    if (!map.getLayer("earthquake-stations-layer")) {
      map.addLayer({
        id: "earthquake-stations-layer",
        type: "circle",
        source: "earthquake-stations",
        paint: {
          "circle-radius": 5,
    ...
    if (!map.getLayer("earthquake-epicenters-layer")) {
      map.addLayer({
        id: "earthquake-epicenters-layer",
        type: "circle",
        source: "earthquake-epicenters",
        paint: {
          "circle-radius": ["interpolate", ["linear"], ["to-number", ["get", "magnitude_value"]], 3, 8, 5, 13, 6, 18],
          "circle-color": ["get", "color"],
          "circle-stroke-color": "#ffffff",
  ...
  useEffect(() => {
    ...
    const view = initialMapView(earthquakesRef.current);
    const map = new maplibregl.Map({ container: mapContainerRef.current, style: osmRasterStyle, center: view.center, zoom: view.zoom, maxZoom: 12 });
    mapRef.current = map;
